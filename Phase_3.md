...
The RequestCount metric can be configured to have [an alarm associated to it](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/AlarmThatSendsEmail.html). When the alarm is configured, there are different evaluation methods that can be chosen. The simplest would be a basic threshold. If the number of requests goes above a certain number an event can be triggered to perform an action.
...
AWS has a [documented solution](https://aws.amazon.com/blogs/containers/autoscaling-amazon-eks-services-based-on-custom-prometheus-metrics-using-cloudwatch-container-insights/) that follows a similar pattern but an event is triggered off of a Prometheus metric.
